-- Query_1 Show skills which baby sitter have and languages spoken (Inner join)
...
-- Query_3 - Determining number of hours worked

-- DATEDIFF WORKS ON MS SQL AND MYSQL VERSION OF DATEDIFF IS DIFFERENT
SELECT FULL_NAME, AVAILABLE_DAY_OF_WEEK,DATEDIFF(hour,cast(AVAILABLE_START_TIME as time), CAST(AVAILABLE_END_TIME as time )) AS NUMBER_OF_HOURS
...
-- Query_4 - Employees who do not have contracts yet  WILL BE SHOWN AS NULL
...
UPDATE FAMILY_EMAIL SET EMAIL = 'ijk@yahoo.com' WHERE FAMILY_ID = 1 AND EMAIL = 'xyy@gmail.com';
...
SELECT CONTRACT_ID FROM CONTRACT WHERE DATEDIFF(DAY, END_DATE,CAST(CURRENT_TIMESTAMP AS date)) > 30

)

DELETE FROM CONTRACT WHERE DATEDIFF(DAY, END_DATE,CAST(CURRENT_TIMESTAMP AS date)) > 30
...
SELECT FULL_NAME, CAST((DATEDIFF(MINUTE,cast(AVAILABLE_START_TIME as time), CAST(AVAILABLE_END_TIME as time))/60.0) AS DECIMAL(18,2)) AS NUMBER_OF_HOURS_PER_DAY,

CAST((DATEDIFF(MINUTE,cast(AVAILABLE_START_TIME as time), CAST(AVAILABLE_END_TIME as time))/60.0) AS DECIMAL(18,2)) * HOURLY_PAY_RATE AS ESTIMATED_MONTHLY_PAY

FROM

BABY_SITTER

GROUP BY FULL_NAME,CAST((DATEDIFF(MINUTE,cast(AVAILABLE_START_TIME as time), CAST(AVAILABLE_END_TIME as time))/60.0) AS DECIMAL(18,2)),

CAST((DATEDIFF(MINUTE,cast(AVAILABLE_START_TIME as time), CAST(AVAILABLE_END_TIME as time))/60.0) AS DECIMAL(18,2)) * HOURLY_PAY_RATE
...
SELECT BRANCH_ID, FULL_NAME, CAST((DATEDIFF(MINUTE,cast(AVAILABLE_START_TIME as time), CAST(AVAILABLE_END_TIME as time))/60.0) AS DECIMAL(18,2)) AS NUMBER_OF_HOURS_PER_DAY,
...
GROUP BY BRANCH_ID,FULL_NAME,CAST((DATEDIFF(MINUTE,cast(AVAILABLE_START_TIME as time), CAST(AVAILABLE_END_TIME as time))/60.0) AS DECIMAL(18,2)),
...
WHEN RANK() OVER(ORDER BY DATEDIFF(hour,cast(AVAILABLE_START_TIME as time), CAST(AVAILABLE_END_TIME as time )) DESC) = 1 THEN 'Top performer'

WHEN RANK() OVER(ORDER BY DATEDIFF(hour,cast(AVAILABLE_START_TIME as time), CAST(AVAILABLE_END_TIME as time ))) = 1 THEN 'Least Performer'
